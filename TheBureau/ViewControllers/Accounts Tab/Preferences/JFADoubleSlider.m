...
#define KNOB_STROKE_COLOR [UIColor redColor];
#define KNOB_FILL_COLOR [UIColor redColor];
static const CGFloat LINE_WIDTH = 2.0;
static const CGFloat KNOB_BORDER_WIDTH = 1.0;
static const CGFloat KNOB_WIDTH = 27.0;
    ...
    if (gesture.state == UIGestureRecognizerStateChanged || (!self.continuous && gesture.state == UIGestureRecognizerStateEnded))
    {
        ...
        if (self.currentKnob == LEFT_KNOB)
        {
                ...
                (newKnobVal < self.curMinVal || newKnobVal < self.curMaxVal - self.minValSpan))
            {
                ...
                {
                    if (self.tag == 999) {
                        self.curMinVal =  self.curMaxVal - newKnobVal > 4 ? newKnobVal : self.curMinVal;
                    }
                    else if (self.tag == 888) {
                        self.curMinVal =  self.curMaxVal - newKnobVal > 4 ? newKnobVal : self.curMinVal;
                    }
                    else {
                        self.curMinVal = self.curMinVal;
                    }
                }
                else
                {
        ...
        else
        {
                ...
                (newKnobVal > self.curMaxVal || newKnobVal > self.curMinVal + self.minValSpan))
            {
                ...
                {
                    if (self.tag == 999) {
                        self.curMaxVal = newKnobVal - self.curMinVal > 4 ? newKnobVal : self.curMaxVal;
                    }
                    else if (self.tag == 888) {
                        self.curMaxVal = newKnobVal - self.curMinVal > 4 ? newKnobVal : self.curMaxVal;
                    }
                    else {
                        self.curMaxVal = newKnobVal > 49.9 ? newKnobVal : self.curMaxVal;
                    }
                }
                else
                {
...
- (void)drawRect:(CGRect)rect
{
    ...
    /////// here to change the textValue of the label
...
- (void)drawKnobAtX:(CGFloat)x value:(float)value halfBoundsHeight:(float)halfBoundsHeight
{
    ...
    if (self.showValues)
    {
        ...
        if (self.reportInteger)
        {
            ...
            if (self.tag == 888) {
                NSInteger feet =  4 + (NSInteger)lroundf(value) / 12;
                NSInteger inch =  (NSInteger)lroundf(value) % 12;
                inch = inch < 0 ? 0 : inch;
                valueString = [NSString stringWithFormat:@"%ld' %ld\"",(long)feet,(long)inch];
            }
        ...
        else
        {
            ...
            if (self.tag == 888) {
                NSInteger feet =  4 + (NSInteger)lroundf([valueString floatValue]) / 12;
                NSInteger inch =  (NSInteger)lroundf([valueString floatValue]) % 12;
                inch = inch < 0 ? 0 : inch;
                valueString = [NSString stringWithFormat:@"%ld' %ld\"",(long)feet,(long)inch];
            }
        }
